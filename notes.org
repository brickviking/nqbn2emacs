...
*** Key: =A-=
  + Long name: Alt
  + Note: This key on the keyboard actually issues =Meta= keystroke, not an =Alt= keystroke. As a result, there aren't many Emacs keybindings (if any) that use Alt-something. They generally get shown as Meta-something.
