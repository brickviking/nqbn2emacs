...
*** =C-Home= - =beginning-of-buffer=
  ...
  + With optional arg between 0 and 10, positions cursor arg*10 percent
...
These commands all work with windows within a frame. A frame is a collection of one or more buffers,
divided among one or more windows. A window is a "division" of the frame into one or more, usually
showing the contents of a buffer. Not all buffers have to be visible, but loaded buffers can usually
...
frames are collections of buffers organised amongst one or more windows per frame. Most of these commands
rely on a graphical display being present (either of Xorg/Wayland/Aqua/Windows) as opposed to a purely
textual display (as in a terminal emulator).
