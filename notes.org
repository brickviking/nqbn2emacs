...
*** Open a ssh file on another computer
I always forget how to open a file over ssh, so I'm including this file after the fact.
One of the help files I stumbled across in Google said that tramp access uses urls that look like
the following: tramp /access:user@host:/path/to/file.

This would mean (at least for accessing a file remotely over ssh) a link that looks like this:
#+BEGIN_EXAMPLE
tramp /ssh:madman@tencity:/home/madman/quicknote.txt
#+END_EXAMPLE

There are other access methods tramp knows how to use, but this is all I really needed for my use.
