...
*** info
This is a help format that's used internally for the help documentation of Emacs. It's a cut-down version
of TeX called texinfo, which is then compiled to create an info document for use with the standalone info
command or the internal help (=C-h i=). It contains formatting, links and a syntax for moving the cursor around between sections that's much like the [[#emacs-movement-keys][Emacs movement keys]] section below.
