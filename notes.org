...
** Registers
Registers are slots to store stuff in, whether it's a string or a file position. This section would
benefit from being filled in further, though it's well described in info documentation
(=C-h i=, =m=, =Emacs=, =m=, =Registers=).
...
For example, let's say I notice an error on the word "troibles" in the following quote:
#+BEGIN_EXAMPLE
...
As stated above, this jumps you back to a previously-stored position in register <x>. Your previous position before jumping will also be stored, so if you need to get there, you can get back with =C-x C-x=.
