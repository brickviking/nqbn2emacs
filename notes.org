...
(defun source-find (&optional sf-fn)
  "When called interactively, requests optional relative path/filename stored in source-directory.
You can provide either a path or a path and filename.
...
When called from inside a lisp buffer (C-x C-e), takes optional relative path to a filename in
double-quotes."
  (interactive)
  (if (null sf-fn)
  (find-file  ;; t-clause (sf-fn has nothing in it)
     (concat
      source-directory
      (completing-read
       "Open source file: "
   ...
   (find-file (concat source-directory sf-fn)) ;; f-clause (sf-fn has a value, even if empty)
))
