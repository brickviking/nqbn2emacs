...
used a reasonably recent version of vim (8.1) but not as up-to-date as Emacs. Most of the Vim things
should work all the way back to perhaps 6.0, though there may be some differences.
...
:CONTENTS:
- [[#emacs][Emacs]]
  - [[#common-emacs-terms][Common Emacs terms]]
  - [[#emacs-key-definitions][Emacs keyboard definitions]]
  - [[#rare-modifier-keys][Rare modifier keys]]
  - [[#mouse-buttons][Mouse buttons]]
  - [[#emacs-movement-keys][Emacs movement keys]]
  - [[#emacs-editing-keys][Editing keys]]
  - [[#indenting][Indenting]]
  ...
  - [[#snippets_2][Additional things I've looked up]]
...
- [[#vim][Vim]]
  - [[#vim-short-example][Short example]]
  - [[#vim-movement-keys][Vim movement keys]]
  - [[#vim-help][Vim Help]]
...
* Emacs                                                               :emacs:
  ...
  :CUSTOM_ID: emacs
  :END:
** Common Emacs terms                                    :common_emacs_terms:
  :PROPERTIES:
  :CUSTOM_ID: common-emacs-terms
  :END:
These are several of the terms in common use inside the Emacs help, and other reference
material. Most of them have equivalents in other editors. Other terms that I haven't used
here will be defined later, as we refer to them. This will include terms like window, buffer and
frame.

*** Point
You'll have seen mention of the term "point". This simply means the place in the current file where
your cursor (that blinky or block thing) is right now. In some cases, it's the gap between
characters, in some other places, it's the character you're sitting on.

*** Mark (=C-Space=)
It's used frequently along with the point, to create a region (or selection) to use for future stuff
like cutting, replacing, undoing or other editing tasks. Use the =C-Space= key to set it. You'll
then see a selection appear when you move the cursor away from that location.

*** Region
Often called the selection, it's the area of lines between a mark (set by =C-Space= earlier) and
your current cursor location (point).

*** Info
This is a help format that's used internally for the help documentation of Emacs, as well as for
system information on UNIX and Unix-like systems. It's written in a cut-down version of TeX called
texinfo, which is then compiled to create an info document for use with the standalone info command
or the internal help (=C-h i=). It uses [[#emacs-movement-keys][Emacs movement keys]] to move around, and supports various
ways to format text. An info document can also contain links to other content, both within the same
document, or to other documents in info format.

*** Mode
Info link: [[info:emacs#Modes][Modes]]

At least for me, the term "mode" is used rather loosely to mean a way to present information to the
user (yes, that's you) that's specific to the file you're currently looking at. In the case of
programming languages, there's usually a "major" mode in control that may colour certain keywords
differently from the rest of the text. Examples usually include keywords, comments or other things
related to the language family the mode covers. You can have one major mode per type of file.

In addition, you can also add minor modes by toggling them on or off. These can include simple
things like toggling line wrapping, highlighting all searches in a document, or many other things.

There are far better ways to describe what "mode" means, at least in the context of the Emacs
editor, and the best description can probably be found in the Info docs supplied with Emacs.

** What different keys mean                        :emacs-key-definitions:
  :PROPERTIES:
  :CUSTOM_ID: emacs-key-definitions
  :END:
...
and b. <RET> is simply the =Return= key, or =Enter= on most modern keyboards. It's not usually the
Enter/Return key on the far right in the number pad section, though that often does the same thing
for convenience.
...
** Rare modifier keys                                    :rare_modifier_keys:
  :PROPERTIES:
...
*** Key: =A-=
  + Long name: Alt
  + Note: On almost every modern keyboard, the key with this name actually issues the =Meta= keystroke,
     not an Emacs =Alt= keystroke.
    As a result, there aren't many Emacs keybindings that use Alt-something, and most commands use
    Meta-something.
*** Key: =s-=
  + Long name: Super
  + Alternative: Windows
  + Alternative: Command (MacOS X)
  + Note: Often used by the operating system especially on Windows and Mac OS X
  + This is not represented with the uppercase S used for Shift keys, but a lowercase s.
*** Key: =H-=
  + Long name: Hyper
...
** Emacs movement keys                                  :emacs_movement_keys:
  :PROPERTIES:
  :CUSTOM_ID: emacs-movement-keys
  :END:

Info link: [[info:emacs#Moving point][Moving point]]

Vim has them. Bash even has some of these. Most modern editors have Up/Down/Left/Right to move by
individual lines and characters, and C-right/left to move forward/backwards by word, but Emacs and Vim both
...
*** =M-g g <num>= =goto-line=
  + Step the cursor to the line specified, i.e. =M-g g 335= moves point to line 335 if available.
*** =C-Home= - =beginning-of-buffer=
  ...
  + With optional arg between 0 and 10, (=C-u arg C-Home=) positions cursor arg*10 percent
    ...
    For example, =C-u 5 C-Home= would mean to jump to 50% into the file.
*** =C-End= - =end-of-buffer=
  ...
  + With optional arg between 0 and 10, (=C-u arg C-End=) positions cursor arg*10 percent
    ...
    For example, =C-u 3 C-End= would mean to jump to 30% from the end of the file.

** Editing keys                                                :emacs_editing_keys:
  :PROPERTIES:
  :CUSTOM_ID: emacs-editing-keys
  :END:
...
** Display of long lines                              :display_of_long_lines:
  :PROPERTIES:
...
|-----------------------+------------------------+--------------------------------------|
| Call (or run) the last-defined macro    | =C-x e=                | kmacro-end-and-call-macro            |
|-----------------------+------------------------+--------------------------------------|
...
** Emacs help                                                    :emacs_help:
  :PROPERTIES:
  :CUSTOM_ID: emacs-help
  :END:

Info link: [[info:emacs#Help][Help]]

What follows is a rough summary of the available help commands with Emacs function names where
present. The first is pretty important and may help you out even if you don't remember the
others. More complete details are available from the info manual.

*** =C-h C-h=  (help-for-help)
Metahelp: a front page to other help pages.
*** =C-h C-c= - Copyright notice (describe-copying)
Displays the copyright notice for Emacs, in this case, the GNU General Public License version 3.
*** =C-h C-a= (about-emacs)
This displays a screen describing this install of Emacs, which usually includes how and when it was
compiled.
...
split buffer usually to the right or below your current buffer.
...
This shortcut may be rebound if you ever load the Hyperbole package, so use
=M-x view-hello-file= if that is the case.
*** The long way around
...
** Packages recommended by others            :packages_recommended_by_others:
  :PROPERTIES:
...
+ =C-c C-v C-t= (or simply, C-c C-v t) Tangle source code blocks to relevant file.  This means
  ...
  =init.el= from an =emacs.org= file, with all the bits I want to fire out to there inside
...
** Snippets from freenode#emacs and libera#emacs                 :snippets_1:
  :PROPERTIES:
...
(defun window-split-four ()
 " Splits frame into four equal sized windows"
  ...
  (split-window))
#+END_SRC
*** How to set a buffer's line-encoding from text mode :how_to_set_a_buffers_line_encoding_from_text_mode:
:PROPERTIES:
...
** Additional things I've looked up                              :snippets_2:
  :PROPERTIES:
  :CUSTOM_ID: snippets_2
  :END:
These are some other things that I've added to the list of tips that I've thought would be really
useful for others to know about, as they may not have come across them yet. This list will be
modified as I find more useful things.

*** How to insert output from a shell command into a buffer.
This seems to be a question asked a few times at stackoverflow, so I've grabbed one of the most
common examples here: =C-u M-! <shell-command>=

Thanks for this reply from https://stackoverflow.com/users/347380/anupam.
*** How to toggle a buffer's read-only state (=C-x C-q=)
This is useful for making sure that a buffer doesn't get modified by accident, at least until
you reverse the read-only setting. If a buffer was read-only when you opened it, it's probably
not a good idea to change that, as it's likely to be a buffer that Emacs will overwrite for itself
such as the Messages window or Emacs sourcetree files, which are normally compressed and
won't take kindly to being changed.
** tias moments (Try It And See)                                       :tias:
  :PROPERTIES:
  :CUSTOM_ID: tias
  :END:
...
** Other useful links, channels and the like             :other_useful_links:
  :PROPERTIES:
...
** Things to find out about                        :things_to_find_out_about:
  :PROPERTIES:
...
The commands I'll describe are relevant to most available versions of Vim past 6.0, though I've
focussed on things available in the 8.1 version.

** Short example                                          :vim_short_example:
  :PROPERTIES:
  :CUSTOM_ID: vim-short-example
  :END:
...
use them, so I don't include much here about plugins. I also don't describe more detailed vimscript
here, basically because I don't know how to use it myself.

** Vim movement keys                                      :vim_movement_keys:
  :PROPERTIES:
  :CUSTOM_ID: vim-movement-keys
  :END:
...
** Vim help                                                        :vim_help:
  ...
  |-----------------+--------------------+---------------------------------------------------------|
** Opening files                                          :opening_files_vim:
  :PROPERTIES:
...
Check :help edit
   ...
   |---------------------------------+---------------+--------------------------------------------|
** Diffing files                                          :diffing_files_vim:
  :PROPERTIES:
...
** Copying commands                                    :copying_commands_vim:
  :PROPERTIES:
...
** Window commands                                      :window_commands_vim:
  :PROPERTIES:
  :CUSTOM_ID: window-commands-vim
  :END:

Window commands operate to open/close or move between existing windows. In essence, a Vim window is like an Emacs buffer, in
...
** Macro commands                                        :macro_commands_vim:
  :PROPERTIES:
...
** Exit commands                                          :exit_commands_vim:
  :PROPERTIES:
...
** Codemy (normally payware) Video                            :codemy_videos:
  :PROPERTIES:
...
** FreeCodeCamp.org Videos  [[https://youtube.com/c/freecodecamp][Youtube Channel]] :freecodecamp_org_videos:
  :PROPERTIES:
...
*** SQL
    ...
    + [[https://www.youtube.com/watch?v=xh4gy1lbL2k][NoSQL overview - full course for beginners (2:54:52)]]
**** Postgresql
     + [[https://www.youtube.com/watch?v=qw--VYLpxG4][Learn PostGreSQL Tutorial (4:19:34)]]
** Other Videos                                    :other_programming_videos:
  :PROPERTIES:
...
* Useful sites
** A note about transience
Here's a small discussion I found about deliberate transience of working state as a mindset.
...
** Atmospheric/ambient/Dark                               :music_atmospheric:
  :PROPERTIES:
  ...
  + Nils Frahm - Live concert at the Philharmonie de Paris, March 2024
    + https://www.youtube.com/watch?v=HPPzQgTaLbo
** Programming music                                      :music_programming:
  :PROPERTIES:
...
** Game Soundtracks                                       :music_soundtracks:
  :PROPERTIES:
