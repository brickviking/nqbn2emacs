...
there. If you redefine source-directory in your init.el, don't forget to put the / on the end.
It won't work if you're using an emacs compiled from a Linux distribution's repository
(Ubuntu/Fedora/etc) because the value in source-directory probably won't exist on your own system.
...
;; Only requires that source-directory's is current for your install.
(defun source-find ()
  "Opens provided filename stored in source-directory, takes a relative path"
  (interactive)
  (find-file
     (concat
      source-directory
      (completing-read
       "Open source file: "
       (directory-files source-directory
                        nil
